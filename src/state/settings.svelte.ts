...
/**
 * Держать ли markdown колонкой читаемой ширины по центру окна (Р-156).
 *
 * Умолчание `true` повторяет умолчание ядра — по той же причине, что
 * у панели разметки: иначе первые полсекунды после запуска текст стоял бы
 * во всю ширину, а потом прыгал в середину.
 */
