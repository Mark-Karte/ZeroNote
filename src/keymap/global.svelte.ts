...
/**
 * Сочетания, которые в некоторых местах интерфейса означают другое.
 *
 * Пока одно: `F2` в тексте — переход к следующей закладке, а в дереве файлов —
 * переименование (Р-117). Это не исключение из правила «все сочетания
 * в реестре», а признание того, что часть клавиш контекстна по своей природе:
 * `Tab` уже такой (Р-115), и в VS Code `F2` тоже.
 *
 * Ключ — команда, значение — селектор места, которому она уступает.
 */
const CONTEXTUAL: Record<string, string> = {
  'view.bookmark-next': '.zn-tree',
};
...
const WEBVIEW_DEFAULTS = new Set([
  ...
  'ctrl+j',
  'ctrl+shift+r',
  // Масштаб. До задачи 41 эти две строки были написаны знаками — `ctrl+=`
  // и `ctrl+-`, — а разбор нажатия таких имён не выдаёт никогда: он называет
  // клавишу по положению. Совпасть они не могли, и масштаб вебвью
  // на Ctrl+«+» менялся, вопреки написанному здесь (Р-121).
  'ctrl+equal',
  'ctrl+minus',
  'ctrl+0',
]);
...
export function installGlobalKeymap(onProblems: (problems: string[]) => void): () => void {
  const onKeyDown = (event: KeyboardEvent): void => {
    ...
    if (command) {
      ...
      // Место, где у этой клавиши своё значение, разбирается само.
      const yieldTo = CONTEXTUAL[command];
      if (yieldTo && event.target instanceof Element && event.target.closest(yieldTo)) {
        return;
      }
