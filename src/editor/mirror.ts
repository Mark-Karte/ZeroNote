...
/**
 * Расширение зеркала: **истории текста у него нет, история курсора — есть.**
 *
 * Отмена правки в зеркале идёт через главное состояние — как в VS Code,
 * где история у документа, а не у окна: две истории на один текст разошлись
 * бы после первого же `Ctrl+Z` не в той области. Поэтому всякая транзакция,
 * меняющая документ, в историю зеркала не попадает.
 *
 * А движения курсора попадают, и это задача 98 (долг этапа 11). Курсор
 * у зеркала свой — так решено в Р-209 и подтверждено задачей 84, — значит
 * и «вернуть курсор туда, где он был» здесь своё. Раньше история зеркала
 * была пуста целиком, и `edit.undo-cursor` в нём молчал: отменять было
 * нечего, а перенаправить команду в главное значило бы двигать чужой
 * курсор в другой области.
 */
export const mirrorOnly: Extension = EditorState.transactionExtender.of((tr) =>
  tr.docChanged ? { annotations: Transaction.addToHistory.of(false) } : null,
);
