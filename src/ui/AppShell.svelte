<script lang="ts">
  ...
  /**
   * Что показывать в рабочей области, решает вид активной вкладки (Р-185).
   *
   * Отдельного признака «открыты ли параметры» больше нет: он разошёлся бы
   * с полосой вкладок молча, а вкладка и признак — это два места, где
   * записано одно и то же.
   */
  const activeKind = $derived(activeTab()?.meta.kind ?? 'text');
