...
<style>
  ...
  /* Кнопки окна — во всю высоту шапки, как у системных окон. Сетка
     центрирует ячейки по вертикали, и без `stretch` ячейка сжималась
     до высоты значка: подсветка кнопки выходила узкой полоской (найдено
     владельцем после 0.16.0, сломано задачей 102). `flex` нужен затем,
     что `align-self: stretch` у самих кнопок действует только внутри
     flex-контейнера. */
  .right {
    display: flex;
    justify-self: end;
