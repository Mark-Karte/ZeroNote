...
/// Версия 3 — имена всех файлов, а не только текстовых (задача 82): у строки
/// появился признак `has_text`, а сама строка теперь заводится и картинке,
...
fn create(connection: &Connection) -> Result<(), rusqlite::Error> {
    connection.execute_batch(
        ...
        CREATE TABLE files (
            ...
            name       TEXT NOT NULL,
            -- Прочитано ли содержимое файла в таблицу `content` (задача 82).
            --
            -- Строка в `files` есть у любого файла проекта — картинки, PDF,
            -- шрифта; содержимое читается только у текстовых, не превышающих
            -- предел из настроек проекта. Признак нужен потребителям, которые
            -- спрашивают не «какие файлы есть», а «на что можно сослаться»:
            -- поиск по проекту отсеивает такие файлы сам, соединяясь
            -- с `content`, а разрешению ссылок и подсказке имён нужен
            -- отдельный ответ.
