...
pub const BASE: &[(&str, &str)] = &[
    ...
    // Поле страницы при печати (задача 109). Плотность на бумагу не влияет,
    // поэтому здесь, а не в метриках. Значение подставляет в `@page` сама
    // печать: переменные CSS внутри `@page` не действуют.
    ("space-print-page", "20mm"),
...
pub const SEMANTIC_COLORS: &[(&str, &str)] = &[
    ...
    // Подложка утопленного блока внутри текста: блок кода, карточка callout.
    // Тот же цвет, что `bg-canvas`, но **с прозрачностью**: слой выделения
    // CodeMirror лежит под содержимым, и сплошная подложка закрывала бы
    // выделение внутри блока целиком (задача 100). Значение выводится
    // из палитры, тема может задать `bg-block` сама.
    ("color-bg-block", "{palette.bg-block}"),
