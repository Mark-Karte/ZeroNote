...
impl DataDir {
    ...
    /// Куда класть ежедневные заметки, если папка не задана настройкой.
    ///
    /// Своя папка, а не `drafts`: черновики принадлежат механизму
    /// восстановления (инвариант 4) и удаляются вместе с буфером, которому
    /// они принадлежали. Заметка человека, положенная туда, исчезла бы
