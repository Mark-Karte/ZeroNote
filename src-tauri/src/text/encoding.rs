...
impl Encoding {
    ...
    /// Однобайтовая ли кодировка.
    ///
    /// Нужно там, где решается, можно ли доверить выбор подсказке проекта:
    /// эвристика ошибается именно на однобайтовых, а UTF-16 без метки узнаётся
    /// по нулевым байтам почти наверняка, и подменять этот ответ нельзя.
    pub fn is_single_byte(self) -> bool {
        matches!(
            self,
            Encoding::Windows1251 | Encoding::Windows1252 | Encoding::Ibm866 | Encoding::Koi8R
        )
    }
