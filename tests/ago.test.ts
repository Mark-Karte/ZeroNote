...
describe('русское числительное', () => {
  it('склоняет по остатку, но одиннадцать особое', async () => {
    ...
    expect(форма(5)).toBe('5 курсоров');
    // Одиннадцать — не «одиннадцать курсор», хотя остаток от десяти равен одному.
    expect(форма(11)).toBe('11 курсоров');
    expect(форма(12)).toBe('12 курсоров');
    expect(форма(21)).toBe('21 курсор');
    expect(форма(22)).toBe('22 курсора');
    expect(форма(111)).toBe('111 курсоров');
  });
});
