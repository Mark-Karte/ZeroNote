...
describe('живое превью: знаки вокруг текста', () => {
  ...
  /**
   * Пробел за скобкой знаку принадлежит один. В коде и во вложенном пункте
   * дальше — отступ, и он остаётся: до приёмки этапа 17 код в коллауте
   * на экране терял отступы.
   */
  it('в коде и во вложенном пункте цитаты отступ остаётся', () => {
    const code = '> ```\n> def f():\n>     return 1\n> ```\n\n';
    expect(shown(code, code.length)).toBe('```\ndef f():\n    return 1\n```\n\n');
    const list = '> - пункт\n>   - вложенный\n\n';
    expect(shown(list, list.length)).toBe('- пункт\n  - вложенный\n\n');
  });

  it('лишние пробелы за скобкой в тексте цитаты прячутся', () => {
    const doc = '>   текст\n\n';
    expect(shown(doc, doc.length)).toBe('текст\n\n');
  });
  ...
  /**
   * В коде ссылок нет (Р-069). Вики-ссылки превью ищет по тексту, и до
   * этой проверки `[[nodiscard]]` в блоке C++ показывался словом
   * `nodiscard` — превью меняло показанный код.
   */
  it('двойные скобки в коде — код, а не вики-ссылка', () => {
    const doc = 'текст\n\n```cpp\n[[nodiscard]] int f();\n```\n\nи `a[[i]]` в строке\n';
    expect(shown(doc, 0)).toBe(
      'текст\n\n```cpp\n[[nodiscard]] int f();\n```\n\nи a[[i]] в строке\n',
    );
  });
