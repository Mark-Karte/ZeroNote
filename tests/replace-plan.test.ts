...
describe('список до записи', () => {
  ...
  /**
   * Список файлов показывается целиком, пока его можно прочитать: это
   * единственное место, где человек видит, во что ввязывается. Правило
   * пришло из переименования (Р-136).
   */
  it('не обрезает список файлов, пока он читаем', () => {
  ...
  /**
   * Но предел у списка есть, и это находка приёмки этапа 13: замена
   * по пяти тысячам файлов выводила пять тысяч строк, диалог перерастал
   * экран, и кнопок не было видно вовсе. Список из пяти тысяч имён не читают;
   * сколько их всего, сказано первой строкой, и она всегда на виду.
   */
