...
- Поиск по содержимому (`Ctrl+Shift+F`) на SQLite FTS5: строится в фоне,
  отменяется, обновляется сам по изменениям на диске. Можно искать
  и регулярным выражением.
- Замена по всему проекту или по одной папке: список изменений показывается
  **до** записи, файл с несохранёнными правками не трогается, пять последних
  замен отменяются.
...
- Живое превью markdown: знаки разметки прячутся, а строка под курсором
  показывается как написана — править можно всегда. Файл при этом
  не меняется ни на байт; переключатель «превью / исходник» — в строке
  состояния.
- Таблицы сеткой, картинки в тексте, коллауты `> [!tip]` карточками — список
  коллаутов свой: подпись, цвет и значок правятся. Задачи `- [ ]` отмечаются
  щелчком.
- Формулы `$…$` и `$$…$$`, как в Obsidian. Рисует их движок окна шрифтом
  Cambria Math, который есть в каждой Windows.
- Схемы mermaid: блок ` ```mermaid ` показывается схемой в цветах темы.
...
- Печать, экспорт в PDF одной командой и экспорт в HTML одним файлом —
  стиль, картинки, формулы и схемы внутри, открывается в любом браузере.
- Копирование с оформлением — в письмо или Word: формулы становятся
  формулами Word, схемы — картинками.
- Бумага всегда светлая, даже если на экране тёмная тема.
...
- Панель инструментов над текстом, состав и порядок кнопок — настройкой.
  Разметка markdown **переключается**: нажатое второй раз снимается.
- Картинки и PDF открываются своей вкладкой, с масштабом и переходом
  по страницам.
...
- Горячие клавиши переназначаются во вкладке «Клавиши» окна параметров или
  ...
  из VS Code.
- Тема выбирается во вкладке «Оформление» окна параметров — карточкой
  с образцом на настоящих цветах. Там же «создать свою на основе этой»,
  редактор цветов и размеров темы и шрифты интерфейса и текста. Сохранили
  файл темы — окно перерисовалось, перезапуск не нужен.
- Опечатка в файле настроек не отключает весь файл: незнакомое называется
  строкой вверху окна, всё остальное применяется.
...
Пока нет, но обсуждается: граф связей, мини-карта.
...
окно занимается отображением. PDF показывает pdf.js, формулы — Temml, схемы —
mermaid; все три грузятся, только когда понадобились, и на запуск не влияют.
...
В окно приложения собраны библиотеки под открытыми лицензиями. Крупнейшие:
**pdf.js** (Apache-2.0, © Mozilla Foundation), **Temml** (MIT, © 2020 Ron Kok),
**mermaid** (MIT, © Knut Sveidqvist) со своими зависимостями — они под MIT,
...
Формулы рисует системный шрифт Cambria Math; из-за этого буквы
`\mathscr` выглядят так же, как `\mathcal`.
